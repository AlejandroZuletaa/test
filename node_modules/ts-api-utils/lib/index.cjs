...
function canHaveTrailingTrivia(token) {
  switch (token.kind) {
    case import_typescript2.default.SyntaxKind.CloseBraceToken:
      return (
        token.parent.kind !==
          import_typescript2.default.SyntaxKind.JsxExpression ||
...
function isJsxElementOrFragment(node) {
  return (
    node.kind === import_typescript2.default.SyntaxKind.JsxElement ||
    node.kind === import_typescript2.default.SyntaxKind.JsxFragment
  );
}
function forEachComment(node, callback, sourceFile = node.getSourceFile()) {
  ...
  const notJsx =
    ...
    import_typescript2.default.LanguageVariant.JSX;
  return forEachToken(
    node,
    (token) => {
      ...
      if (token.kind !== import_typescript2.default.SyntaxKind.JsxText) {
        import_typescript2.default.forEachLeadingCommentRange(
          ...
          token.pos === 0
            ? (import_typescript2.default.getShebang(fullText) ?? '').length
            : token.pos,
          commentCallback,
        );
      }
    ...
    case 'allowSyntheticDefaultImports':
      return options.allowSyntheticDefaultImports !== void 0
        ...
        : isCompilerOptionEnabled(options, 'esModuleInterop') ||
            options.module === import_typescript3.default.ModuleKind.System;
...
function isModifierFlagSet(node, flag) {
  return isFlagSet(
    import_typescript4.default.getCombinedModifierFlags(node),
    flag,
  );
}
...
function isAssignmentKind(kind) {
  return (
    kind >= import_typescript5.default.SyntaxKind.FirstAssignment &&
    kind <= import_typescript5.default.SyntaxKind.LastAssignment
  );
}
...
function isValidPropertyAccess(
  text,
  languageVersion = import_typescript5.default.ScriptTarget.Latest,
) {
  if (text.length === 0) {
    return false;
  }
  ...
  switch (parent.kind) {
    ...
    case import_typescript6.default.SyntaxKind.PrefixUnaryExpression:
      return parent.operator ===
        import_typescript6.default.SyntaxKind.PlusPlusToken ||
        parent.operator ===
          import_typescript6.default.SyntaxKind.MinusMinusToken
        ? 3 /* ReadWrite */
        : 1 /* Read */;
      ...
      return parent.right === node
        ...
        : !isAssignmentKind(parent.operatorToken.kind)
          ...
          : parent.operatorToken.kind ===
              import_typescript6.default.SyntaxKind.EqualsToken
            ? 2 /* Write */
            : 3 /* ReadWrite */;
    ...
    case import_typescript6.default.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token ===
        import_typescript6.default.SyntaxKind.ExtendsKeyword &&
        parent.parent.parent.kind !==
          import_typescript6.default.SyntaxKind.InterfaceDeclaration
        ? 1 /* Read */
        : 0 /* None */;
...
function isInDestructuringAssignment(node) {
  switch (node.kind) {
    ...
    case import_typescript6.default.SyntaxKind.SpreadElement:
      if (
        node.parent.kind !==
        import_typescript6.default.SyntaxKind.ArrayLiteralExpression
      ) {
        return false;
      }
      node = node.parent;
...
function isEqualsGreaterThanToken(node) {
  return (
    node.kind === import_typescript7.default.SyntaxKind.EqualsGreaterThanToken
  );
}
...
function isJsonMinusNumericLiteral(node) {
  return (
    node.kind === import_typescript7.default.SyntaxKind.PrefixUnaryExpression
  );
}
...
function isUnparsedSyntheticReference(node) {
  return (
    node.kind ===
    import_typescript7.default.SyntaxKind.UnparsedSyntheticReference
  );
}
...
var [tsMajor, tsMinor] = import_typescript8.default.versionMajorMinor
  .split('.')
...
function isAccessExpression(node) {
  return (
    import_typescript9.default.isPropertyAccessExpression(node) ||
    import_typescript9.default.isElementAccessExpression(node)
  );
}
...
function isAccessorDeclaration(node) {
  return (
    import_typescript9.default.isGetAccessorDeclaration(node) ||
    import_typescript9.default.isSetAccessorDeclaration(node)
  );
}
function isArrayBindingElement(node) {
  return (
    import_typescript9.default.isBindingElement(node) ||
    import_typescript9.default.isOmittedExpression(node)
  );
}
function isArrayBindingOrAssignmentPattern(node) {
  return (
    import_typescript9.default.isArrayBindingPattern(node) ||
    import_typescript9.default.isArrayLiteralExpression(node)
  );
}
function isAssignmentPattern(node) {
  return (
    import_typescript9.default.isObjectLiteralExpression(node) ||
    import_typescript9.default.isArrayLiteralExpression(node)
  );
}
function isBindingOrAssignmentElementRestIndicator(node) {
  if (
    import_typescript9.default.isSpreadElement(node) ||
    import_typescript9.default.isSpreadAssignment(node)
  ) {
    return true;
  }
...
function isBindingOrAssignmentElementTarget(node) {
  return (
    ...
    import_typescript9.default.isIdentifier(node) ||
    import_typescript9.default.isPropertyAccessExpression(node) ||
    import_typescript9.default.isElementAccessExpression(node) ||
    import_typescript9.default.isOmittedExpression(node)
  );
...
function isBindingPattern(node) {
  return (
    import_typescript9.default.isObjectBindingPattern(node) ||
    import_typescript9.default.isArrayBindingPattern(node)
  );
}
function isBlockLike(node) {
  return (
    import_typescript9.default.isSourceFile(node) ||
    import_typescript9.default.isBlock(node) ||
    import_typescript9.default.isModuleBlock(node) ||
    import_typescript9.default.isCaseOrDefaultClause(node)
  );
}
...
function isClassLikeDeclaration(node) {
  return (
    import_typescript9.default.isClassDeclaration(node) ||
    import_typescript9.default.isClassExpression(node)
  );
}
...
function isDeclarationName(node) {
  return (
    import_typescript9.default.isIdentifier(node) ||
    import_typescript9.default.isPrivateIdentifier(node) ||
    import_typescript9.default.isStringLiteralLike(node) ||
    import_typescript9.default.isNumericLiteral(node) ||
    import_typescript9.default.isComputedPropertyName(node) ||
    import_typescript9.default.isElementAccessExpression(node) ||
...
function isDeclarationWithTypeParameterChildren(node) {
  return (
    isSignatureDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    import_typescript9.default.isInterfaceDeclaration(node) ||
    import_typescript9.default.isTypeAliasDeclaration(node) ||
    import_typescript9.default.isJSDocTemplateTag(node)
  );
}
function isDeclarationWithTypeParameters(node) {
  return (
    isDeclarationWithTypeParameterChildren(node) ||
    import_typescript9.default.isJSDocTypedefTag(node) ||
    import_typescript9.default.isJSDocCallbackTag(node) ||
    import_typescript9.default.isJSDocSignature(node)
  );
}
...
function isEntityNameOrEntityNameExpression(node) {
  return (
    import_typescript9.default.isEntityName(node) ||
    isEntityNameExpression(node)
  );
}
function isForInOrOfStatement(node) {
  return (
    import_typescript9.default.isForInStatement(node) ||
    import_typescript9.default.isForOfStatement(node)
  );
}
function isFunctionLikeDeclaration(node) {
  return (
    import_typescript9.default.isFunctionDeclaration(node) ||
    import_typescript9.default.isMethodDeclaration(node) ||
    import_typescript9.default.isGetAccessorDeclaration(node) ||
    import_typescript9.default.isSetAccessorDeclaration(node) ||
    import_typescript9.default.isConstructorDeclaration(node) ||
    import_typescript9.default.isFunctionExpression(node) ||
    import_typescript9.default.isArrowFunction(node)
  );
}
function hasDecorators(node) {
  return (
    import_typescript9.default.isParameter(node) ||
    import_typescript9.default.isPropertyDeclaration(node) ||
    import_typescript9.default.isMethodDeclaration(node) ||
    import_typescript9.default.isGetAccessorDeclaration(node) ||
    import_typescript9.default.isSetAccessorDeclaration(node) ||
    import_typescript9.default.isClassExpression(node) ||
...
function hasExpressionInitializer(node) {
  return (
    import_typescript9.default.isVariableDeclaration(node) ||
    import_typescript9.default.isParameter(node) ||
    import_typescript9.default.isBindingElement(node) ||
    import_typescript9.default.isPropertyDeclaration(node) ||
    import_typescript9.default.isPropertyAssignment(node) ||
    import_typescript9.default.isEnumMember(node)
  );
}
function hasInitializer(node) {
  return (
    hasExpressionInitializer(node) ||
    import_typescript9.default.isForStatement(node) ||
    import_typescript9.default.isForInStatement(node) ||
    import_typescript9.default.isForOfStatement(node) ||
    import_typescript9.default.isJsxAttribute(node)
  );
}
function hasJSDoc(node) {
  if (
    ...
    import_typescript9.default.isArrowFunction(node) ||
    import_typescript9.default.isBlock(node) ||
    import_typescript9.default.isBreakStatement(node) ||
    import_typescript9.default.isCallSignatureDeclaration(node) ||
    import_typescript9.default.isCaseClause(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    import_typescript9.default.isConstructorDeclaration(node) ||
    import_typescript9.default.isConstructorTypeNode(node) ||
    import_typescript9.default.isConstructSignatureDeclaration(node) ||
    import_typescript9.default.isContinueStatement(node) ||
    import_typescript9.default.isDebuggerStatement(node) ||
    import_typescript9.default.isDoStatement(node) ||
    import_typescript9.default.isEmptyStatement(node) ||
    isEndOfFileToken(node) ||
    import_typescript9.default.isEnumDeclaration(node) ||
    import_typescript9.default.isEnumMember(node) ||
    import_typescript9.default.isExportAssignment(node) ||
    import_typescript9.default.isExportDeclaration(node) ||
    import_typescript9.default.isExportSpecifier(node) ||
    import_typescript9.default.isExpressionStatement(node) ||
    import_typescript9.default.isForInStatement(node) ||
    import_typescript9.default.isForOfStatement(node) ||
    import_typescript9.default.isForStatement(node) ||
    import_typescript9.default.isFunctionDeclaration(node) ||
    import_typescript9.default.isFunctionExpression(node) ||
    import_typescript9.default.isFunctionTypeNode(node) ||
    import_typescript9.default.isIfStatement(node) ||
    import_typescript9.default.isImportDeclaration(node) ||
    import_typescript9.default.isImportEqualsDeclaration(node) ||
    import_typescript9.default.isIndexSignatureDeclaration(node) ||
    import_typescript9.default.isInterfaceDeclaration(node) ||
    import_typescript9.default.isJSDocFunctionType(node) ||
    import_typescript9.default.isLabeledStatement(node) ||
    import_typescript9.default.isMethodDeclaration(node) ||
    import_typescript9.default.isMethodSignature(node) ||
    import_typescript9.default.isModuleDeclaration(node) ||
    import_typescript9.default.isNamedTupleMember(node) ||
    import_typescript9.default.isNamespaceExportDeclaration(node) ||
    import_typescript9.default.isParameter(node) ||
    import_typescript9.default.isParenthesizedExpression(node) ||
    import_typescript9.default.isPropertyAssignment(node) ||
    import_typescript9.default.isPropertyDeclaration(node) ||
    import_typescript9.default.isPropertySignature(node) ||
    import_typescript9.default.isReturnStatement(node) ||
    import_typescript9.default.isShorthandPropertyAssignment(node) ||
    import_typescript9.default.isSpreadAssignment(node) ||
    import_typescript9.default.isSwitchStatement(node) ||
    import_typescript9.default.isThrowStatement(node) ||
    import_typescript9.default.isTryStatement(node) ||
    import_typescript9.default.isTypeAliasDeclaration(node) ||
    import_typescript9.default.isVariableDeclaration(node) ||
    import_typescript9.default.isVariableStatement(node) ||
    import_typescript9.default.isWhileStatement(node) ||
    import_typescript9.default.isWithStatement(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(4, 4) &&
    import_typescript9.default.isClassStaticBlockDeclaration(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(5, 0) &&
    (import_typescript9.default.isBinaryExpression(node) ||
      import_typescript9.default.isElementAccessExpression(node) ||
      import_typescript9.default.isIdentifier(node) ||
      import_typescript9.default.isJSDocSignature(node) ||
      import_typescript9.default.isObjectLiteralExpression(node) ||
      import_typescript9.default.isPropertyAccessExpression(node) ||
      import_typescript9.default.isTypeParameterDeclaration(node))
  ) {
    return true;
  }
  return false;
...
function hasType(node) {
  return (
    ...
    import_typescript9.default.isPropertySignature(node) ||
    import_typescript9.default.isPropertyDeclaration(node) ||
    import_typescript9.default.isTypePredicateNode(node) ||
    import_typescript9.default.isParenthesizedTypeNode(node) ||
    import_typescript9.default.isTypeOperatorNode(node) ||
    import_typescript9.default.isMappedTypeNode(node) ||
    import_typescript9.default.isAssertionExpression(node) ||
    import_typescript9.default.isTypeAliasDeclaration(node) ||
    import_typescript9.default.isJSDocTypeExpression(node) ||
    import_typescript9.default.isJSDocNonNullableType(node) ||
    import_typescript9.default.isJSDocNullableType(node) ||
    import_typescript9.default.isJSDocOptionalType(node) ||
    import_typescript9.default.isJSDocVariadicType(node)
  );
}
function hasTypeArguments(node) {
  return (
    import_typescript9.default.isCallExpression(node) ||
    import_typescript9.default.isNewExpression(node) ||
    import_typescript9.default.isTaggedTemplateExpression(node) ||
    import_typescript9.default.isJsxOpeningElement(node) ||
    import_typescript9.default.isJsxSelfClosingElement(node)
  );
}
function isJSDocComment(node) {
  ...
  if (isTsVersionAtLeast(4, 4)) {
    return (
      import_typescript9.default.isJSDocLink(node) ||
      import_typescript9.default.isJSDocLinkCode(node) ||
      import_typescript9.default.isJSDocLinkPlain(node)
    );
  }
  return false;
}
function isJSDocNamespaceBody(node) {
  return (
    import_typescript9.default.isIdentifier(node) ||
    isJSDocNamespaceDeclaration(node)
  );
}
...
function isJsonObjectExpression(node) {
  return (
    ...
    import_typescript9.default.isNumericLiteral(node) ||
    import_typescript9.default.isStringLiteral(node) ||
...
function isJsxAttributeLike(node) {
  return (
    import_typescript9.default.isJsxAttribute(node) ||
    import_typescript9.default.isJsxSpreadAttribute(node)
  );
}
function isJsxAttributeValue(node) {
  return (
    import_typescript9.default.isStringLiteral(node) ||
    import_typescript9.default.isJsxExpression(node) ||
    import_typescript9.default.isJsxElement(node) ||
    import_typescript9.default.isJsxSelfClosingElement(node) ||
    import_typescript9.default.isJsxFragment(node)
  );
}
function isJsxChild(node) {
  return (
    import_typescript9.default.isJsxText(node) ||
    import_typescript9.default.isJsxExpression(node) ||
    import_typescript9.default.isJsxElement(node) ||
    import_typescript9.default.isJsxSelfClosingElement(node) ||
    import_typescript9.default.isJsxFragment(node)
...
function isLiteralToken(node) {
  return (
    import_typescript9.default.isNumericLiteral(node) ||
    import_typescript9.default.isBigIntLiteral(node) ||
    import_typescript9.default.isStringLiteral(node) ||
    import_typescript9.default.isJsxText(node) ||
    import_typescript9.default.isRegularExpressionLiteral(node) ||
    import_typescript9.default.isNoSubstitutionTemplateLiteral(node)
  );
}
...
function isModuleReference(node) {
  return (
    import_typescript9.default.isEntityName(node) ||
    import_typescript9.default.isExternalModuleReference(node)
  );
}
function isNamedImportBindings(node) {
  return (
    import_typescript9.default.isNamespaceImport(node) ||
    import_typescript9.default.isNamedImports(node)
  );
}
function isNamedImportsOrExports(node) {
  return (
    import_typescript9.default.isNamedImports(node) ||
    import_typescript9.default.isNamedExports(node)
  );
}
...
function isObjectTypeDeclaration(node) {
  return (
    ...
    isClassLikeDeclaration(node) ||
    import_typescript9.default.isInterfaceDeclaration(node) ||
    import_typescript9.default.isTypeLiteralNode(node)
  );
}
...
function isPseudoLiteralToken(node) {
  return (
    import_typescript9.default.isTemplateHead(node) ||
    import_typescript9.default.isTemplateMiddle(node) ||
    import_typescript9.default.isTemplateTail(node)
  );
}
function isSignatureDeclaration(node) {
  return (
    ...
    import_typescript9.default.isFunctionDeclaration(node) ||
    import_typescript9.default.isMethodDeclaration(node) ||
    import_typescript9.default.isConstructorDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    import_typescript9.default.isFunctionExpression(node) ||
    import_typescript9.default.isArrowFunction(node)
  );
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  if (
    import_typescript9.default.isImportClause(node) ||
    import_typescript9.default.isImportEqualsDeclaration(node) ||
    import_typescript9.default.isNamespaceImport(node) ||
    import_typescript9.default.isImportOrExportSpecifier(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(5, 0) &&
    (import_typescript9.default.isExportDeclaration(node) ||
      import_typescript9.default.isNamespaceExport(node))
  ) {
    return true;
  }
  return false;
...
function isTypeReferenceType(node) {
  return (
    import_typescript9.default.isTypeReferenceNode(node) ||
    import_typescript9.default.isExpressionWithTypeArguments(node)
  );
}
function isUnionOrIntersectionTypeNode(node) {
  return (
    import_typescript9.default.isUnionTypeNode(node) ||
    import_typescript9.default.isIntersectionTypeNode(node)
  );
}
function isUnparsedSourceText(node) {
  return (
    import_typescript9.default.isUnparsedPrepend(node) ||
    import_typescript9.default.isUnparsedTextLike(node)
  );
}
function isVariableLikeDeclaration(node) {
  return (
    ...
    import_typescript9.default.isShorthandPropertyAssignment(node) ||
    import_typescript9.default.isEnumMember(node) ||
    import_typescript9.default.isJSDocPropertyTag(node) ||
    import_typescript9.default.isJSDocParameterTag(node)
  );
}
...
function isConstAssertionExpression(node) {
  return (
    import_typescript10.default.isTypeReferenceNode(node.type) &&
    import_typescript10.default.isIdentifier(node.type.typeName) &&
...
function isJSDocNamespaceDeclaration(node) {
  return (
    import_typescript10.default.isModuleDeclaration(node) &&
    import_typescript10.default.isIdentifier(node.name) &&
    (node.body === void 0 || isJSDocNamespaceBody(node.body))
  );
}
function isJsxTagNamePropertyAccess(node) {
  return (
    import_typescript10.default.isPropertyAccessExpression(node) && // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts < 5
...
function isPropertyAccessEntityNameExpression(node) {
  return (
    import_typescript10.default.isPropertyAccessExpression(node) &&
    import_typescript10.default.isIdentifier(node.name) &&
...
function isSuperElementAccessExpression(node) {
  return (
    import_typescript10.default.isElementAccessExpression(node) &&
...
var IntrinsicTypeFlags =
  import_typescript12.default.TypeFlags.Intrinsic ??
  import_typescript12.default.TypeFlags.Any |
    import_typescript12.default.TypeFlags.Unknown |
    import_typescript12.default.TypeFlags.String |
    import_typescript12.default.TypeFlags.Number |
    import_typescript12.default.TypeFlags.BigInt |
    import_typescript12.default.TypeFlags.Boolean |
    import_typescript12.default.TypeFlags.BooleanLiteral |
    import_typescript12.default.TypeFlags.ESSymbol |
    import_typescript12.default.TypeFlags.Void |
    import_typescript12.default.TypeFlags.Undefined |
    import_typescript12.default.TypeFlags.Null |
    import_typescript12.default.TypeFlags.Never |
    import_typescript12.default.TypeFlags.NonPrimitive;
...
function isIndexedAccessType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.IndexedAccess,
  );
}
function isInstantiableType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.Instantiable,
  );
}
function isIntersectionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.Intersection,
  );
}
...
function isStringMappingType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.StringMapping,
  );
}
function isSubstitutionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.Substitution,
  );
}
function isTypeParameter(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.TypeParameter,
  );
}
function isTypeVariable(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.TypeVariable,
  );
}
...
function isUnionOrIntersectionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.UnionOrIntersection,
  );
}
function isUniqueESSymbolType(type) {
  return isTypeFlagSet(
    type,
    import_typescript13.default.TypeFlags.UniqueESSymbol,
  );
}
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript14.default.ObjectFlags.EvolvingArray)
  );
}
function isTupleType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript14.default.ObjectFlags.Tuple)
  );
}
function isTypeReference(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript14.default.ObjectFlags.Reference)
  );
}
...
function isBooleanLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript15.default.TypeFlags.BooleanLiteral,
  );
}
function isBigIntLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript15.default.TypeFlags.BigIntLiteral,
  );
}
...
function isNumberLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript15.default.TypeFlags.NumberLiteral,
  );
}
function isStringLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript15.default.TypeFlags.StringLiteral,
  );
}
function isTemplateLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript15.default.TypeFlags.TemplateLiteral,
  );
}
...
function getWellKnownSymbolPropertyOfType(
  ...
  for (const prop of type.getProperties()) {
    ...
    if (
      ...
      !import_typescript16.default.isComputedPropertyName(declaration.name)
    ) {
      continue;
    }
...
function isBindableObjectDefinePropertyCall(node) {
  return (
    ...
    import_typescript17.default.isPropertyAccessExpression(node.expression) &&
    node.expression.name.escapedText === 'defineProperty' &&
    import_typescript17.default.isIdentifier(node.expression.expression) &&
  ...
  while (true) {
    ...
    outer: switch (parent.kind) {
      ...
      case import_typescript17.default.SyntaxKind.PrefixUnaryExpression:
        if (
          current.kind !== import_typescript17.default.SyntaxKind.NumericLiteral
        ) {
          return false;
        }
...
function isFalsyType(type) {
  if (
    isTypeFlagSet(
      type,
      import_typescript18.default.TypeFlags.Undefined |
        import_typescript18.default.TypeFlags.Null |
        import_typescript18.default.TypeFlags.Void,
    )
  ) {
    return true;
  }
  ...
  return typeParts2.some((subType) => {
    ...
    return !!(
      ...
      (
        isSymbolFlagSet(
          prop,
          import_typescript18.default.SymbolFlags.ValueModule,
...
function isReadonlyPropertyFromMappedType(type, name, typeChecker) {
  if (
    !isObjectType(type) ||
    !isObjectFlagSet(type, import_typescript18.default.ObjectFlags.Mapped)
  ) {
    return;
  }
  ...
  if (declaration.readonlyToken !== void 0 && !/^__@[^@]+$/.test(name)) {
    return (
      ...
      import_typescript18.default.SyntaxKind.MinusToken
    );
  }
      ...
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(
              subType,
              import_typescript18.default.IndexKind.Number,
            )
          : void 0) ??
        typeChecker.getIndexInfoOfType(
          subType,
          import_typescript18.default.IndexKind.String,
        );
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    ...
    import_typescript18.default.isPropertyAssignment(
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & import_typescript18.default.SymbolFlags.Accessor) ===
      import_typescript18.default.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      (node) =>
        isModifierFlagSet(
          node,
          import_typescript18.default.ModifierFlags.Readonly,
        ) ||
        (import_typescript18.default.isVariableDeclaration(node) &&
          isNodeFlagSet(
            node.parent,
            import_typescript18.default.NodeFlags.Const,
          )) ||
        (import_typescript18.default.isCallExpression(node) &&
          isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        import_typescript18.default.isEnumMember(node) ||
        ((import_typescript18.default.isPropertyAssignment(node) ||
          import_typescript18.default.isShorthandPropertyAssignment(node)) &&
...
function isBlockScopeBoundary(node) {
  switch (node.kind) {
    case import_typescript19.default.SyntaxKind.Block: {
      const parent = node.parent;
      return parent.kind !==
        import_typescript19.default.SyntaxKind.CatchClause && // blocks inside SourceFile are block scope boundaries
...
function identifierToKeywordKind(node) {
  return 'identifierToKeywordKind' in import_typescript20.default
    ? import_typescript20.default.identifierToKeywordKind(node)
    : // eslint-disable-next-line deprecation/deprecation
      node.originalKeywordKind;
}
function canHaveDecorators(node) {
  return 'canHaveDecorators' in import_typescript20.default
    ? import_typescript20.default.canHaveDecorators(node)
    : 'decorators' in node;
}
function getDecorators(node) {
  return 'getDecorators' in import_typescript20.default
    ? import_typescript20.default.getDecorators(node)
    : node.decorators;
}
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    ...
    case import_typescript21.default.SyntaxKind.Parameter:
      if (
        node.parent.parent.kind ===
          import_typescript21.default.SyntaxKind.IndexSignature ||
        identifierToKeywordKind(node) ===
          import_typescript21.default.SyntaxKind.ThisKeyword
      ) {
        return;
      }
...
function unwrapParentheses(node) {
  while (
    node.kind === import_typescript22.default.SyntaxKind.ParenthesizedExpression
  ) {
...
function getPropertyName(propertyName) {
  if (
    propertyName.kind ===
    import_typescript22.default.SyntaxKind.ComputedPropertyName
  ) {
    ...
    if (import_typescript22.default.isPrefixUnaryExpression(expression)) {
      ...
      switch (expression.operator) {
        ...
        case import_typescript22.default.SyntaxKind.PlusToken:
          return import_typescript22.default.isNumericLiteral(
            expression.operand,
          )
            ? `${negate ? '-' : ''}${expression.operand.text}`
            : import_typescript22.default.isBigIntLiteral(expression.operand)
  ...
  return propertyName.kind ===
    import_typescript22.default.SyntaxKind.PrivateIdentifier
    ? void 0
    : propertyName.text;
}
  ...
  switch (parent.kind) {
    case import_typescript23.default.SyntaxKind.TypeReference:
      return identifierToKeywordKind(node) !==
        import_typescript23.default.SyntaxKind.ConstKeyword
        ? 2 /* Type */
        : void 0;
    case import_typescript23.default.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token ===
        import_typescript23.default.SyntaxKind.ImplementsKeyword ||
        parent.parent.parent.kind ===
          import_typescript23.default.SyntaxKind.InterfaceDeclaration
        ? 2 /* Type */
        : 4 /* Value */;
    ...
    case import_typescript23.default.SyntaxKind.QualifiedName:
      if (parent.left === node) {
        if (
          ...
          import_typescript23.default.SyntaxKind.TypeQuery
        ) {
...
var RootScope = class extends AbstractScope {
  ...
  addVariable(identifier, name, selector, exported, domain) {
    ...
    __privateGet(this, _innerScope).end((value, key) => {
      value.exported ||=
        __privateGet(this, _exportAll) ||
        (__privateGet(this, _exports) !== void 0 &&
          __privateGet(this, _exports).includes(key.text));
...
var NamespaceScope = class extends NonRootScope {
  ...
  createOrReuseEnumScope(name, exported) {
    if (
      !exported &&
      (!__privateGet(this, _ambient) || __privateGet(this, _hasExport))
    ) {
      return __privateGet(this, _innerScope2).createOrReuseEnumScope(
        name,
        exported,
      );
    }
  ...
  createOrReuseNamespaceScope(name, exported, ambient, hasExportStatement) {
    ...
    __privateGet(this, _innerScope2).end((variable, key, scope) => {
      if (
        scope !== __privateGet(this, _innerScope2) ||
        (!variable.exported &&
          (!__privateGet(this, _ambient) ||
            (__privateGet(this, _exports2) !== void 0 &&
              !__privateGet(this, _exports2).has(key.text))))
      ) {
...
var AbstractNamedExpressionScope = class extends NonRootScope {
  ...
  addUse(use, source) {
    ...
    if (
      use.domain & __privateGet(this, _domain) &&
      use.location.text === __privateGet(this, _name).text
    ) {
      this.uses.push(use);
    } else {
      return this.parent.addUse(use, this);
...
var _result,
  _scope,
  _handleBindingName,
  handleBindingName_fn,
  _handleConditionalType,
  handleConditionalType_fn,
  _handleDeclaration,
  handleDeclaration_fn,
  _handleFunctionLikeDeclaration,
  handleFunctionLikeDeclaration_fn,
  _handleModule,
  handleModule_fn,
  _handleVariableDeclaration,
  handleVariableDeclaration_fn;
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
    ...
    const cb = (node) => {
      if (isBlockScopeBoundary(node)) {
        return continueWithScope(
          node,
          new BlockScope(
            __privateGet(this, _scope).getFunctionScope(),
      ...
      switch (node.kind) {
        case import_typescript24.default.SyntaxKind.ClassExpression:
          return continueWithScope(
            node,
            node.name !== void 0
              ? new ClassExpressionScope(node.name, __privateGet(this, _scope))
        ...
        case import_typescript24.default.SyntaxKind.ModuleDeclaration:
          return __privateMethod(this, _handleModule, handleModule_fn).call(
            this,
            node,
            continueWithScope,
          );
        ...
        case import_typescript24.default.SyntaxKind.FunctionType:
          return __privateMethod(
            ...
            _handleFunctionLikeDeclaration,
            handleFunctionLikeDeclaration_fn,
          ).call(this, node, cb, variableCallback);
        ...
        case import_typescript24.default.SyntaxKind.Parameter:
          if (
            node.parent.kind !==
              import_typescript24.default.SyntaxKind.IndexSignature &&
            (node.name.kind !==
              import_typescript24.default.SyntaxKind.Identifier ||
            ...
            __privateMethod(
              ...
              _handleBindingName,
              handleBindingName_fn,
            ).call(this, node.name, false, false);
          }
          break;
        ...
        case import_typescript24.default.SyntaxKind.TypeParameter:
          __privateGet(this, _scope).addVariable(
            ...
            node.name,
            node.parent.kind ===
              import_typescript24.default.SyntaxKind.InferType
              ? 8 /* InferType */
              : 7 /* Type */,
            false,
            2 /* Type */,
        ...
        case import_typescript24.default.SyntaxKind.ExportAssignment:
          if (
            ...
            return __privateGet(this, _scope).markExported(node.expression);
          }
          break;
    ...
    const handleBlockScope = (node) => {
      if (
        node.kind === import_typescript24.default.SyntaxKind.CatchClause &&
        node.variableDeclaration !== void 0
      ) {
        __privateMethod(this, _handleBindingName, handleBindingName_fn).call(
          this,
...
handleConditionalType_fn = function (node, cb, varCb) {
  const savedScope = __privateGet(this, _scope);
  const scope = __privateSet(
    this,
    _scope,
    new ConditionalTypeScope(savedScope),
  );
  cb(node.checkType);
...
handleFunctionLikeDeclaration_fn = function (node, cb, varCb) {
  ...
  const savedScope = __privateGet(this, _scope);
  if (
    node.kind === import_typescript24.default.SyntaxKind.FunctionDeclaration
  ) {
    __privateMethod(this, _handleDeclaration, handleDeclaration_fn).call(
      this,
      node,
  ...
  const scope = __privateSet(
    this,
    _scope,
    node.kind === import_typescript24.default.SyntaxKind.FunctionExpression &&
      node.name !== void 0
...
handleVariableDeclaration_fn = function (declarationList) {
  ...
  const exported =
    declarationList.parent.kind ===
      import_typescript24.default.SyntaxKind.VariableStatement &&
    includesModifier(
...
function isNamespaceExported(node) {
  return (
    node.parent.kind ===
      import_typescript24.default.SyntaxKind.ModuleDeclaration ||
    includesModifier(
      node.modifiers,
      import_typescript24.default.SyntaxKind.ExportKeyword,
    )
...
function namespaceHasExportStatement(ns) {
  if (
    ns.body === void 0 ||
    ns.body.kind !== import_typescript24.default.SyntaxKind.ModuleBlock
  ) {
    return false;
  }
...
function containsExportStatement(block) {
  for (const statement of block.statements) {
    if (
      statement.kind ===
        import_typescript24.default.SyntaxKind.ExportDeclaration ||
      statement.kind === import_typescript24.default.SyntaxKind.ExportAssignment
    ) {
      return true;
    }
  }
...
function isBlockScopedVariableDeclarationList(declarationList) {
  return (
    (declarationList.flags &
      import_typescript24.default.NodeFlags.BlockScoped) !==
    0
  );
}
function forEachDestructuringIdentifier(pattern, fn) {
  for (const element of pattern.elements) {
    if (
      element.kind !== import_typescript24.default.SyntaxKind.BindingElement
    ) {
      continue;
    }
    let result;
    if (
      element.name.kind === import_typescript24.default.SyntaxKind.Identifier
    ) {
      result = fn(element);
    } else {
