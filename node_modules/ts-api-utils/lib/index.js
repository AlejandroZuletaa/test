...
function canHaveTrailingTrivia(token) {
  switch (token.kind) {
    case ts2.SyntaxKind.CloseBraceToken:
      return (
        token.parent.kind !== ts2.SyntaxKind.JsxExpression ||
...
function isJsxElementOrFragment(node) {
  return (
    node.kind === ts2.SyntaxKind.JsxElement ||
    node.kind === ts2.SyntaxKind.JsxFragment
  );
}
    ...
    case 'allowSyntheticDefaultImports':
      return options.allowSyntheticDefaultImports !== void 0
        ...
        : isCompilerOptionEnabled(options, 'esModuleInterop') ||
            options.module === ts3.ModuleKind.System;
...
function isAssignmentKind(kind) {
  return (
    kind >= ts5.SyntaxKind.FirstAssignment &&
    kind <= ts5.SyntaxKind.LastAssignment
  );
}
...
function isValidPropertyAccess(
  text,
  languageVersion = ts5.ScriptTarget.Latest,
) {
  if (text.length === 0) {
    return false;
  }
...
function getAccessKind(node) {
  ...
  switch (parent.kind) {
    ...
    case ts6.SyntaxKind.PrefixUnaryExpression:
      return parent.operator === ts6.SyntaxKind.PlusPlusToken ||
        parent.operator === ts6.SyntaxKind.MinusMinusToken
        ? 3 /* ReadWrite */
        : 1 /* Read */;
    case ts6.SyntaxKind.BinaryExpression:
      return parent.right === node
        ...
        : !isAssignmentKind(parent.operatorToken.kind)
          ? 1 /* Read */
          : parent.operatorToken.kind === ts6.SyntaxKind.EqualsToken
            ? 2 /* Write */
            : 3 /* ReadWrite */;
    ...
    case ts6.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token === ts6.SyntaxKind.ExtendsKeyword &&
        parent.parent.parent.kind !== ts6.SyntaxKind.InterfaceDeclaration
        ? 1 /* Read */
        : 0 /* None */;
    ...
    switch (node.parent.kind) {
      case ts6.SyntaxKind.BinaryExpression:
        return (
          ...
          node.parent.operatorToken.kind === ts6.SyntaxKind.EqualsToken
        );
      case ts6.SyntaxKind.ForOfStatement:
...
var [tsMajor, tsMinor] = ts8.versionMajorMinor
  .split('.')
...
function hasJSDoc(node) {
  if (
    ...
    ts9.isCaseClause(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    ts9.isConstructorDeclaration(node) ||
...
function isSignatureDeclaration(node) {
  return (
    ...
    ts9.isFunctionDeclaration(node) ||
    ts9.isMethodDeclaration(node) ||
    ts9.isConstructorDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    ts9.isFunctionExpression(node) ||
    ts9.isArrowFunction(node)
  );
...
function isConstAssertionExpression(node) {
  return (
    ts10.isTypeReferenceNode(node.type) &&
    ts10.isIdentifier(node.type.typeName) &&
...
function isJSDocNamespaceDeclaration(node) {
  return (
    ts10.isModuleDeclaration(node) &&
    ts10.isIdentifier(node.name) &&
    (node.body === void 0 || isJSDocNamespaceBody(node.body))
  );
}
function isJsxTagNamePropertyAccess(node) {
  return (
    ts10.isPropertyAccessExpression(node) && // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts < 5
...
function isPropertyAccessEntityNameExpression(node) {
  return (
    ts10.isPropertyAccessExpression(node) &&
    ts10.isIdentifier(node.name) &&
...
function isSuperElementAccessExpression(node) {
  return (
    ts10.isElementAccessExpression(node) && isSuperExpression(node.expression)
  );
}
function isSuperPropertyAccessExpression(node) {
  return (
    ts10.isPropertyAccessExpression(node) && isSuperExpression(node.expression)
  );
}
...
var IntrinsicTypeFlags =
  ts12.TypeFlags.Intrinsic ??
  ts12.TypeFlags.Any |
    ts12.TypeFlags.Unknown |
    ts12.TypeFlags.String |
    ts12.TypeFlags.Number |
    ts12.TypeFlags.BigInt |
    ts12.TypeFlags.Boolean |
    ts12.TypeFlags.BooleanLiteral |
    ts12.TypeFlags.ESSymbol |
    ts12.TypeFlags.Void |
    ts12.TypeFlags.Undefined |
    ts12.TypeFlags.Null |
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts14.ObjectFlags.EvolvingArray)
  );
}
...
function isTypeReference(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts14.ObjectFlags.Reference)
  );
}
...
function getWellKnownSymbolPropertyOfType(
  ...
  for (const prop of type.getProperties()) {
    ...
    if (
      ...
      !ts16.isComputedPropertyName(declaration.name)
    ) {
      continue;
    }
...
function isBindableObjectDefinePropertyCall(node) {
  return (
    ...
    ts17.isPropertyAccessExpression(node.expression) &&
    node.expression.name.escapedText === 'defineProperty' &&
    ts17.isIdentifier(node.expression.expression) &&
    ...
    if (getPropertyOfType(subType, name) === void 0) {
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(subType, ts18.IndexKind.Number)
          : void 0) ??
        typeChecker.getIndexInfoOfType(subType, ts18.IndexKind.String);
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
        }
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    ...
    ts18.isPropertyAssignment(writableProp.valueDeclaration)
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & ts18.SymbolFlags.Accessor) ===
      ts18.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      (node) =>
        isModifierFlagSet(node, ts18.ModifierFlags.Readonly) ||
        (ts18.isVariableDeclaration(node) &&
          isNodeFlagSet(node.parent, ts18.NodeFlags.Const)) ||
        (ts18.isCallExpression(node) &&
          isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        ts18.isEnumMember(node) ||
        ((ts18.isPropertyAssignment(node) ||
          ts18.isShorthandPropertyAssignment(node)) &&
...
function identifierToKeywordKind(node) {
  return 'identifierToKeywordKind' in ts20
    ? ts20.identifierToKeywordKind(node)
    : // eslint-disable-next-line deprecation/deprecation
      node.originalKeywordKind;
}
function canHaveDecorators(node) {
  return 'canHaveDecorators' in ts20
    ? ts20.canHaveDecorators(node)
    : 'decorators' in node;
}
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    ...
    case ts21.SyntaxKind.Parameter:
      if (
        node.parent.parent.kind === ts21.SyntaxKind.IndexSignature ||
        identifierToKeywordKind(node) === ts21.SyntaxKind.ThisKeyword
      ) {
        return;
      }
...
function getPropertyName(propertyName) {
    ...
    if (ts22.isPrefixUnaryExpression(expression)) {
      ...
      switch (expression.operator) {
        ...
        case ts22.SyntaxKind.PlusToken:
          return ts22.isNumericLiteral(expression.operand)
            ? `${negate ? '-' : ''}${expression.operand.text}`
            : ts22.isBigIntLiteral(expression.operand)
  ...
  return propertyName.kind === ts22.SyntaxKind.PrivateIdentifier
    ? void 0
    : propertyName.text;
}
...
function getUsageDomain(node) {
  ...
  switch (parent.kind) {
    case ts23.SyntaxKind.TypeReference:
      return identifierToKeywordKind(node) !== ts23.SyntaxKind.ConstKeyword
        ? 2 /* Type */
        : void 0;
    case ts23.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token === ts23.SyntaxKind.ImplementsKeyword ||
        parent.parent.parent.kind === ts23.SyntaxKind.InterfaceDeclaration
        ? 2 /* Type */
        : 4 /* Value */;
...
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
    ...
    const cb = (node) => {
      ...
      switch (node.kind) {
        ...
        case ts24.SyntaxKind.EnumDeclaration:
          ...
          return continueWithScope(
            ...
            __privateGet(this, _scope).createOrReuseEnumScope(
              node.name.text,
              includesModifier(node.modifiers, ts24.SyntaxKind.ExportKeyword),
            ),
          );
        ...
        case ts24.SyntaxKind.Parameter:
          if (
            node.parent.kind !== ts24.SyntaxKind.IndexSignature &&
            (node.name.kind !== ts24.SyntaxKind.Identifier ||
        ...
        case ts24.SyntaxKind.TypeParameter:
          __privateGet(this, _scope).addVariable(
            node.name.text,
            node.name,
            node.parent.kind === ts24.SyntaxKind.InferType
              ? 8 /* InferType */
              : 7 /* Type */,
            false,
            2 /* Type */,
    ...
    const handleBlockScope = (node) => {
      if (
        node.kind === ts24.SyntaxKind.CatchClause &&
...
handleFunctionLikeDeclaration_fn = function (node, cb, varCb) {
  ...
  const scope = __privateSet(
    this,
    _scope,
    node.kind === ts24.SyntaxKind.FunctionExpression && node.name !== void 0
...
handleVariableDeclaration_fn = function (declarationList) {
  ...
  const exported =
    declarationList.parent.kind === ts24.SyntaxKind.VariableStatement &&
    includesModifier(
...
function isNamespaceExported(node) {
  return (
    node.parent.kind === ts24.SyntaxKind.ModuleDeclaration ||
    includesModifier(node.modifiers, ts24.SyntaxKind.ExportKeyword)
  );
}
...
function containsExportStatement(block) {
  for (const statement of block.statements) {
    if (
      statement.kind === ts24.SyntaxKind.ExportDeclaration ||
      statement.kind === ts24.SyntaxKind.ExportAssignment
    ) {
      return true;
    }
  }
